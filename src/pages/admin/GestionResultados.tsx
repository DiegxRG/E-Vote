// pages/admin/GestionResultados.tsx

import { useState } from 'react';
import { FaChartBar, FaLock, FaClock, FaTrophy, FaEye } from 'react-icons/fa';
import { motion } from 'framer-motion';

// --- DEFINICIÓN DE INTERFACES (Mejora la legibilidad) ---
interface Result {
  id: string;
  fullName: string;
  voteCount: number;
...
function ElectionCard({ election, onCloseElection, isActive }: ElectionCardProps) {
  ...
  return (
            ...
            <div className="flex gap-6 text-sm">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {election.positions.reduce((acc, p) => acc + p.totalVotes, 0).toLocaleString()}
                </p>
              </div>
              <div>
    ...
    // Ordenar los resultados para que el ganador (mayor voteCount) sea el primero
    const sortedResults = [...position.results].sort((a, b) => b.voteCount - a.voteCount);
            ...
            return (
              <div key={position.id}>
                ...
                <div className="space-y-3">
                  {sortedResults.map((result, index: number) => {
                    const percentage = (result.voteCount / position.totalVotes) * 100;
                    const isWinner = index === 0 && position.totalVotes > 0; // El primero es ganador si hay votos
                      ...
                      >
                        ...
                        <div className="relative w-full bg-white rounded-full h-3 overflow-hidden">
                          <motion.div
                            ...
                            initial={{ width: 0 }}
                            animate={{ width: `${(result.voteCount / position.totalVotes) * 100}%` }}
...
export default function GestionResultados() {
  // LÍNEA ELIMINADA: const [selectedElection, setSelectedElection] = useState<string | null>(null);
  ...
  // Calcula votos totales
  const totalGlobalVotes = MOCK_RESULTS.reduce((acc, e) => 
    acc + e.positions.reduce((sum, p) => sum + p.totalVotes, 0), 0
  ).toLocaleString();
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="p-6 md:p-8 bg-slate-100 min-h-full space-y-8"
    >
